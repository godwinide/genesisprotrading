...
router.get("/fund-account", ensureAdmin, async (req, res) => {
    try {
        return res.render("admin/fund", { layout: "admin/layout", pageTitle: "Welcome", req });
    }
    catch (err) {
        return res.redirect("/admin");
    }
...
router.post("/fund-account", ensureAdmin, async (req, res) => {
    try {
        const {username, amount, profit} = req.body;
        if(!username || !amount || !profit){
            req.flash("error_msg", "Please provide all fields");
            return res.redirect("/admin/fund-account");
        }
        ...
        await User.updateOne({ username }, {
            ...
            total_earned: Number(customer.total_earned) + Number(profit)
        });
        req.flash("success_msg", "Funds added to account");
        return res.redirect("/admin/fund-account");
    }
    catch (err) {
        return res.redirect("/admin");
